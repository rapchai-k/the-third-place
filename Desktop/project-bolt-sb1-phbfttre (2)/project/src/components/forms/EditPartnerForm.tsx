...
import { X, Building, MapPin, Mail, Phone, Save } from 'lucide-react';
...
export function EditPartnerForm({ isOpen, partner, onClose, onSuccess }: EditPartnerFormProps) {
  ...
  useEffect(() => {
    if (partner && isOpen) {
      setValue('partner_id', partner.partner_id || '');
      setValue('partner_name', partner.partner_name || '');
      setValue('name', partner.name);
      setValue('partner_city_id', partner.partner_city_id || '');
      setValue('city', partner.city || '');
      setValue('location', partner.location);
      setValue('email', partner.email || '');
      setValue('phone', partner.phone || '');
      setValue('cooperation_status', partner.cooperation_status || '');
      setValue('business_status', partner.business_status || '');
      setValue('capacity_type', partner.capacity_type || '');
      setValue('parent_partner_id', partner.parent_partner_id || '');
      setValue('partner_company_name_en', partner.partner_company_name_en || '');
      setValue('brand_name', partner.brand_name || '');
      setValue('signer_email', partner.signer_email || '');
      setValue('legal_email', partner.legal_email || '');
      setValue('manager_mis_ids', partner.manager_mis_ids || '');
      setValue('region', partner.region || '');
      setValue('is_active', partner.is_active);
    }
  }, [partner, isOpen, setValue]);
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          partner_company_name_en: data.partner_company_name_en,
          brand_name: data.brand_name || null,
          signer_email: data.signer_email || null,
          legal_email: data.legal_email || null,
          manager_mis_ids: data.manager_mis_ids || null,
          region: data.region,
          ...
          <div className="flex space-x-3 pt-4">
            ...
            >
              <div className="flex items-center justify-center space-x-2">
                ...
                <span>{isSubmitting ? 'Updating...' : 'Update Partner'}</span>
              </div>
            </button>
          </div>
        </form>
